...
class FirefightingTourGA:
    ...
    to combat multiple fire ignitions, based on the approach from Matos et al.
    """
    ...
    def _evaluate_fitness(self, chromosome, travel_times, fire_params, base_included=True):
        """
        Evaluate the fitness of a chromosome (tour sequence)
        ...
        for i, fire_idx in enumerate(chromosome):
            ...
            # Get fire parameters
            v0 = fire_params[fire_idx]['initial_area']
            a = fire_params[fire_idx]['deterioration_rate']
            processing_time = self._calculate_processing_time(fire_params[fire_idx], current_time)
            
            # Calculate unburned area using the formula from the paper
            # V[i] = V[i]0 - a[i] * C[i]^2
    ...
    def _calculate_processing_time(self, fire_param, time_elapsed):
        """
        Calculate processing time for a fire based on the paper's formula
        ...
        """
        alpha = fire_param.get('alpha', 0.5)
        beta = fire_param.get('beta', 3.0)
        gamma = fire_param.get('gamma', 0.25)
        delta = fire_param.get('delta', 0.006)
        d = fire_param.get('containment_time', 0.5)
        x = fire_param.get('additional_resources', 1.0)
        
        if time_elapsed <= d:
            # Use the formula from the paper for processing time
            processing_time = (alpha * time_elapsed) / (beta + gamma * time_elapsed + delta)
        else:
            # Use additional resources value
            processing_time = x
    ...
    def _order_crossover(self, parent1, parent2):
        """
        Order-based crossover for permutation representation
    ...
    def _inverse_mutation(self, chromosome):
        """
        Inverse mutation for permutation representation
    ...
    def visualize_solution(self, fire_locations, base_location=None):
        """
        Visualize the optimal tour
        
        Parameters:
        - fire_locations: list of (x, y) coordinates of fire ignitions
        - base_location: (x, y) coordinates of the firefighting resource starting point
                        (if None, will use first fire in the sequence)
        ...
        # Plot fire locations
        x_coords = [loc[0] for loc in fire_locations]
        y_coords = [loc[1] for loc in fire_locations]
        ax.scatter(x_coords, y_coords, color='red', s=100, label='Fire Ignitions')
        
        # Add labels to fire locations
        for i, (x, y) in enumerate(fire_locations):
            ax.text(x, y, f"F{i}", fontsize=12)
        
        # Plot base location if provided
        if base_location is not None:
            ax.scatter(base_location[0], base_location[1], color='blue', s=150, marker='*', label='Base')
            ax.text(base_location[0], base_location[1], "Base", fontsize=12)
            
            # Start path from base
            path_x = [base_location[0]]
            path_y = [base_location[1]]
        else:
            # Start path from first fire in sequence
            first_fire = self.best_solution[0]
            path_x = [fire_locations[first_fire][0]]
            path_y = [fire_locations[first_fire][1]]
        
        # Add points to path in sequence order
        for idx in self.best_solution:
            fire_x, fire_y = fire_locations[idx]
            path_x.append(fire_x)
            path_y.append(fire_y)
        ...
        for i in range(len(path_x)-1):
            ax.arrow(path_x[i], path_y[i], 
                    (path_x[i+1] - path_x[i]) * 0.9, 
                    (path_y[i+1] - path_y[i]) * 0.9,
                    head_width=1, head_length=1, fc='blue', ec='blue')
        
        ax.grid(True)
        ax.set_title("Optimal Firefighting Tour")
...
def apply_ga_tour_strategy(model, steps):
    ...
    if len(fire_locations) <= 1:
        ...
        # Let fire establish slightly (2 steps)
        for _ in range(2):
            model.update()
            history.append(np.copy(model.grid))
        
        # Apply direct attack for remaining steps
        for step in range(steps - 2):
            # Find cells that are currently burning
            burning_cells = np.where(model.grid == 1)
            
            # Apply suppressant to burning cells
            resources_capacity = min(len(burning_cells[0]), 10)  # Can address 10 cells per step
    ...
    for i, (x, y) in enumerate(fire_locations):
        ...
        # Estimate fire parameters based on local conditions
        # These are simplified - in a real application you would use more sophisticated estimation
        ...
        # Containment time estimate (time after which additional resources are needed)
        containment_time = 0.3 + 0.5 * local_ndvi
        
        # Additional resources required if containment time is exceeded
        additional_resources = 0.5 + local_ndvi
        
        # Initial area - use a fraction of the grid size for demonstration
        initial_area = 100 + 50 * local_ndvi
        
        # Deterioration rate (how quickly the area burns)
    ...
    # Visualize the optimal tour (for analysis)
    ga.visualize_solution(fire_locations, base_location)
    plt.savefig('optimal_tour.png')  # Save figure for later reference
