...
def evaluate_model_performance(model, simulation_steps=20, params=None):
    ...
    if params:
        ...
        # Can add humidity coefficient here
        humidity_coeff = params.get('h', -0.0583)
        
        # Set environmental conditions
        model.set_environmental_data(
            wind_speed=params.get('wind_speed', 5.0),
            wind_direction=params.get('wind_direction', 45.0),
            temperature=params.get('temperature', 80.0),
            humidity=params.get('humidity', 20.0),
            fire_direction=params.get('fire_direction', 10.0)
        )
...
class FireModelCalibration(Problem):
    def __init__(self, fire_folder, simulation_steps=20):
        ...
        # Define parameter ranges (6 parameters)
        n_var = 6
        xl = np.array([0.1, 0.1, 0.1, -0.1, 0.0, -0.1])  # Lower bounds for p0, c1, c2, wind_effect, temp_effect, h(humidity)
        xu = np.array([0.9, 1.0, 1.0, 0.0, 0.05, 0.0])   # Upper bounds
    ...
    def _evaluate(self, x, out, *args, **kwargs):
        ...
        for i in range(n_points):
            ...
            params = {
                ...
                'c1': x[i, 1],
                'c2': x[i, 2],
                'wind_speed': 5.0,  # Fixed for now, could be optimized
                'wind_direction': 45.0,  # Fixed for now, could be optimized
                'h': x[i, 5],  # Humidity effect coefficient
                'temperature': 80.0
            }
...
def run_pymoo_optimization(fire_folder, pop_size=30, n_gen=10, simulation_steps=20, algorithm_type="ga"):
    """Run Pymoo optimization to find optimal model parameters"""
    print(f"Running Pymoo optimization for {fire_folder}...")
    ...
    best_params = {
        ...
        'wind_effect': res.X[3],
        'temp_effect': res.X[4],
        'h': res.X[5]  # Humidity effect coefficient
    }
...
# Main execution block - uncomment sections to run
if __name__ == "__main__":
    # Select fire dataset for calibration
    calibration_fire = fires[0]  # alabama
    
    # Run Pymoo optimization
    best_params, best_fitness, res = run_pymoo_optimization(
        fire_folder=calibration_fire,
        pop_size=20,  # Population size
        n_gen=10,     # Number of generations
        simulation_steps=20,
        algorithm_type="ga"  # Use "de" for Differential Evolution
    )
    ...
    # Run sensitivity analysis for humidity coefficient
    h_values, h_scores = parameter_sensitivity_analysis(
        fire_folder=calibration_fire,
        base_params=best_params,
        parameter_name='h',
        value_range=(-0.1, 0.0),  # Range for humidity coefficient
        steps=8,
        simulation_steps=20
    )
