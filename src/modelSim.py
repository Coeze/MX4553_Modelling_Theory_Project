...
class CA:
    ...
    def __init__(self, grid_size=(100, 100), cell_size=1, params=None,  min_elevation=100, max_elevation=1000, scale=100.0, seed=None):
        """
        ...
        self.elevation, self.slope, self.aspect = generate_terrain_simplex(
        self.rows, self.cols, self.cell_size, 
        min_elevation, max_elevation, 
        scale=scale, seed=seed)
        # Generate humidity with simplex noise (more scientifically realistic)
        _, humidity_base, _ = generate_terrain_simplex(
            self.rows, self.cols, self.cell_size,
            min_elevation=20, max_elevation=80,  # 20-80% humidity range
            scale=scale*1.5,  # Larger scale = more gradual changes
            seed=seed+1 if seed is not None else None)  # Different seed for variation
        
        # Adjust humidity based on elevation (higher elevation = typically lower humidity)
        elevation_normalized = (self.elevation - min_elevation) / (max_elevation - min_elevation)
        humidity_adjust = -10 * elevation_normalized  # Higher elevations reduce humidity
        self.humidity = np.clip(humidity_base + humidity_adjust, 20, 80)  # Keep in reasonable range
